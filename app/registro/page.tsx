...
export default function RegistroPage() {
  ...
  // Quick Action: Applica template
  function handleApplyTemplate(templateId: string) {
    const template = getTemplateById(templateId);
    if (!template) return;

    // Aggiorna tipo sessione e fase
    setSessionForm(prev => ({
      ...prev,
      type: template.type,
    ...
    // Applica blocchi esercizi se presenti
    if (template.blocks && template.blocks.length > 0) {
      const newBlocks: ExerciseBlockForm[] = template.blocks.map((block, idx) => ({
    ...
    // Applica metriche se presenti
    if (template.metrics && template.metrics.length > 0) {
      const newMetrics: MetricForm[] = template.metrics.map(m => ({
        date: sessionForm.date,
        metric_name: m.metric_name,
        category: m.category,
        metric_target: '',
        value: '',
        unit: sessionForm.type === 'palestra' ? 'kg' : 's',
        notes: '',
        distance_m: '',
        time_s: '',
        recovery_post_s: '',
    ...
    notifySuccess(`Template "${template.name}" applicato`, {
      description: 'Compila i campi rimanenti e salva',
    });
  }

  // Quick Action: Duplica ultima sessione
  async function handleDuplicateLastSession() {
    const { data, error } = await supabase
    ...
    if (error || !data) {
      notifyError('Nessuna sessione precedente trovata');
      return;
    }

    // Duplica i dati della sessione
    setSessionForm({
    ...
    // Duplica blocchi esercizi
    if (data.exercise_blocks && data.exercise_blocks.length > 0) {
      const duplicatedBlocks: ExerciseBlockForm[] = data.exercise_blocks.map((block: any, idx: number) => ({
        id: `block-${Date.now()}-${idx}`,
        block_number: block.block_number || idx + 1,
        name: block.name || '',
        rest_after_block_s: block.rest_after_block_s?.toString() || '',
        notes: block.notes || '',
        exercises: block.exercises?.map((ex: any) => ({
          ...
          rest_between_reps_s: ex.rest_between_reps_s?.toString() || '',
          rest_between_sets_s: ex.rest_between_sets_s?.toString() || '',
          intensity: ex.intensity?.toString() || '',
          notes: ex.notes || '',
          results: [], // Non duplicare i risultati
        })) || [],
      }));
      setExerciseBlocks(duplicatedBlocks);
    }

    // Duplica metriche
    if (data.metrics && data.metrics.length > 0) {
      const duplicatedMetrics: MetricForm[] = data.metrics.map((m: any) => ({
        date: new Date().toISOString().split('T')[0],
        metric_name: m.metric_name || '',
        category: m.category || 'test',
        metric_target: m.metric_target || '',
        value: '', // Non duplicare il valore
        unit: m.unit || '',
        notes: m.notes || '',
        distance_m: m.distance_m?.toString() || '',
        time_s: '', // Non duplicare il tempo
        recovery_post_s: m.recovery_post_s?.toString() || '',
        intensity: m.intensity?.toString() || '',
      }));
      setMetrics(duplicatedMetrics);
    }

    notifySuccess('Ultima sessione duplicata', {
      description: 'Modifica i dati e salva',
    });
  }

  // Quick Action: Sessione rapida
  function handleQuickAdd() {
    // Reset a una sessione pulita con valori di default
    setSessionForm({
      ...defaultSession,
      date: new Date().toISOString().split('T')[0],
      type: 'pista',
    ...
    notifySuccess('Pronto per inserimento rapido', {
      description: 'Compila solo i campi essenziali',
    });
  }
  ...
  return (
    ...
    >
      ...
      {/* Floating Action Button - Quick Actions (Mobile Only) */}
      <FloatingActionButton
        quickActions={[
          ...
          {
            id: 'duplicate',
            label: 'Duplica Ultima',
            icon: History,
            color: 'bg-purple-500 text-white',
            action: () => void handleDuplicateLastSession(),
          },
          {
            id: 'template-8x100',
            label: '8x100m',
            ...
            action: () => handleApplyTemplate('ripetute-100'),
          },
          {
            id: 'template-5x200',
            label: '5x200m',
            icon: Copy,
            color: 'bg-blue-500 text-white',
            action: () => handleApplyTemplate('ripetute-200'),
          },
          {
            id: 'template-test',
            label: 'Test 60m',
            ...
            action: () => handleApplyTemplate('test-60'),
          },
        ]}
      />
